...
export POSTGRESQL_SHARED_BUFFERS=${POSTGRESQL_SHARED_BUFFERS:-1GB}
export POSTGRESQL_EFFECTIVE_CACHE_SIZE=${POSTGRESQL_EFFECTIVE_CACHE_SIZE:-3GB}
export POSTGRESQL_WORK_MEM=${POSTGRESQL_WORK_MEM:-10485kB}
export POSTGRESQL_MAINTENANCE_WORK_MEM=${POSTGRESQL_MAINTENANCE_WORK_MEM:-256MB}
export POSTGRESQL_CHECKPOINT_SEGMENTS=${POSTGRESQL_CHECKPOINT_SEGMENTS:-32}
export POSTGRESQL_CHECKPOINT_COMPLETION_TARGET=${POSTGRESQL_CHECKPOINT_COMPLETION_TARGET:-0.7}
export POSTGRESQL_WAL_BUFFERS=${POSTGRESQL_WAL_BUFFERS:-16MB}
export POSTGRESQL_DEFAULT_STATISTICS_TARGET=${POSTGRESQL_DEFAULT_STATISTICS_TARGET:-100}
export POSTGRESQL_MAX_WAL_SENDERS=${POSTGRESQL_MAX_WAL_SENDERS:-3}
export POSTGRESQL_WAL_KEEP_SEGMENTS=${POSTGRESQL_WAL_KEEP_SEGMENTS:-400}
...
function usage() {
  ...
  echo >&2 "  POSTGRESQL_SHARED_BUFFERS (default: 1GB)"
  echo >&2 "  POSTGRESQL_EFFECTIVE_CACHE_SIZE (default: 3GB)"
  echo >&2 "  POSTGRESQL_WORK_MEM (default: 10485kB)"
  echo >&2 "  POSTGRESQL_MAINTENANCE_WORK_MEM (default: 256MB)"
  echo >&2 "  POSTGRESQL_CHECKPOINT_SEGMENTS (default: 32)"
  echo >&2 "  POSTGRESQL_CHECKPOINT_COMPLETION_TARGET (default: 0.7)"
  echo >&2 "  POSTGRESQL_WAL_BUFFERS (default: 16MB)"
  echo >&2 "  POSTGRESQL_DEFAULT_STATISTICS_TARGET (default: 100)"
  echo >&2 "  POSTGRESQL_MAX_WAL_SENDERS (default: 3)"
  echo >&2 "  POSTGRESQL_WAL_KEEP_SEGMENTS (default: 400)"
...
function generate_passwd_file() {
  ...
  envsubst < ${CONTAINER_SCRIPTS_PATH}/passwd.template > ${CONTAINER_SCRIPTS_PATH}/passwd
  export LD_PRELOAD=libnss_wrapper.so
  export NSS_WRAPPER_PASSWD=/usr/share/container-scripts/postgresql/passwd
...
function generate_postgresql_recovery_config() {
  envsubst \
      < "${CONTAINER_SCRIPTS_PATH}/recovery.conf.template" \
...
#
# Custom configuration starting at this point.
#
...
function wait_for_postgresql_master() {
  while true; do
    ...
    if [ -v POSTGRESQL_ADMIN_PASSWORD ]; then
      ...
      PGPASSWORD=${POSTGRESQL_PASSWORD} psql "postgresql://${POSTGRESQL_USER}@${master_fqdn}/${POSTGRESQL_DATABASE}" -c "SELECT 1;" && 
return 0
    fi
    sleep 1
  done
